 ...
 *
 * @method TagQuery leftJoinTaskTag($relationAlias = null) Adds a LEFT JOIN clause to the query using the TaskTag relation
 * @method TagQuery rightJoinTaskTag($relationAlias = null) Adds a RIGHT JOIN clause to the query using the TaskTag relation
 * @method TagQuery innerJoinTaskTag($relationAlias = null) Adds a INNER JOIN clause to the query using the TaskTag relation
 *
...
abstract class BaseTagQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = 'App\\CommonBundle\\Model\\Tag', $modelAlias = null)
    {
    ...
    public function filterByTaskTag($taskTag, $comparison = null)
    {
        if ($taskTag instanceof TaskTag) {
            return $this
                ->addUsingAlias(TagPeer::ID, $taskTag->getTagId(), $comparison);
     ...
     *
     * @param   Task $task the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   TagQuery The current query, for fluid interface
     */
    public function filterByTask($task, $comparison = Criteria::EQUAL)
    {
        return $this
            ->useTaskTagQuery()
            ->filterByTask($task, $comparison)
            ->endUse();
    }
