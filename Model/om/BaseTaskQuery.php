 ...
 * @method TaskQuery orderByName($order = Criteria::ASC) Order by the name column
 * @method TaskQuery orderByDescription($order = Criteria::ASC) Order by the description column
 * @method TaskQuery orderByDueDate($order = Criteria::ASC) Order by the due_date column
 *
 * @method TaskQuery groupById() Group by the id column
 * @method TaskQuery groupByName() Group by the name column
 * @method TaskQuery groupByDescription() Group by the description column
 * @method TaskQuery groupByDueDate() Group by the due_date column
 *
 ...
 *
 * @method TaskQuery leftJoinTaskTag($relationAlias = null) Adds a LEFT JOIN clause to the query using the TaskTag relation
 * @method TaskQuery rightJoinTaskTag($relationAlias = null) Adds a RIGHT JOIN clause to the query using the TaskTag relation
 * @method TaskQuery innerJoinTaskTag($relationAlias = null) Adds a INNER JOIN clause to the query using the TaskTag relation
 *
 ...
 *
 * @method Task findOneByName(string $name) Return the first Task filtered by the name column
 * @method Task findOneByDescription(string $description) Return the first Task filtered by the description column
 * @method Task findOneByDueDate(string $due_date) Return the first Task filtered by the due_date column
 *
 * @method array findById(int $id) Return Task objects filtered by the id column
 * @method array findByName(string $name) Return Task objects filtered by the name column
 * @method array findByDescription(string $description) Return Task objects filtered by the description column
 * @method array findByDueDate(string $due_date) Return Task objects filtered by the due_date column
 */
abstract class BaseTaskQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = 'App\\CommonBundle\\Model\\Task', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `ID`, `NAME`, `DESCRIPTION`, `DUE_DATE` FROM `task` WHERE `ID` = :p0';
        try {
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(TaskPeer::NAME, $name, $comparison);
    }
    ...
    public function filterByDueDate($dueDate = null, $comparison = null)
    {
        if (is_array($dueDate)) {
            ...
            if (isset($dueDate['min'])) {
                $this->addUsingAlias(TaskPeer::DUE_DATE, $dueDate['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($dueDate['max'])) {
                $this->addUsingAlias(TaskPeer::DUE_DATE, $dueDate['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(TaskPeer::DUE_DATE, $dueDate, $comparison);
    }

    /**
     * Filter the query by a related TaskTag object
     *
     * @param   TaskTag|PropelObjectCollection $taskTag  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   TaskQuery The current query, for fluid interface
     * @throws   PropelException - if the provided filter is invalid.
     */
    public function filterByTaskTag($taskTag, $comparison = null)
    {
        if ($taskTag instanceof TaskTag) {
            return $this
                ->addUsingAlias(TaskPeer::ID, $taskTag->getTaskId(), $comparison);
        } elseif ($taskTag instanceof PropelObjectCollection) {
            return $this
                ->useTaskTagQuery()
                ->filterByPrimaryKeys($taskTag->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByTaskTag() only accepts arguments of type TaskTag or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the TaskTag relation
     *
     ...
     * @return TaskQuery The current query, for fluid interface
     */
    public function joinTaskTag($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('TaskTag');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'TaskTag');
        }
    ...
    /**
     * Use the TaskTag relation TaskTag object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \App\CommonBundle\Model\TaskTagQuery A secondary query class using the current class as primary query
     */
    public function useTaskTagQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinTaskTag($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'TaskTag', '\App\CommonBundle\Model\TaskTagQuery');
    }

    /**
     * Filter the query by a related Tag object
     * using the task_tag table as cross reference
     *
