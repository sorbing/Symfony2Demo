...
use App\CommonBundle\Model\AuthorQuery;
use App\CommonBundle\Model\Book;
...
abstract class BaseAuthorQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = 'App\\CommonBundle\\Model\\Author', $modelAlias = null)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \App\CommonBundle\Model\BookQuery A secondary query class using the current class as primary query
     */
    public function useBookQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinBook($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Book', '\App\CommonBundle\Model\BookQuery');
    }
